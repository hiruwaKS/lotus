...
def filter_line_1(line):
    # return False
    return line.startswith("[dynaa-check]") 

def main():
    lotus_root = Path(__file__).resolve().parent.parent.parent
    p = argparse.ArgumentParser()
    # ===================== defualt configure =====================
    p.add_argument("--source", default="tmp/test.cpp")
    ...
    p.add_argument("--runtime", default=str(lotus_root / "build/libRuntime.a")+" -lstdc++")
    ...
    # Check each AA
    for aa in args.aa:
        cmd = [args.dynaa_check, str(bc), str(log), aa, "-svf-wpa-temp-dir", args.tmp_dir, "-svf-wpa-path", args.wpa_dir]
        ...
        output = result.stdout + result.stderr
        for line in output.splitlines():
            if filter_line(line) and filter_line_1(line):
                print(line)
